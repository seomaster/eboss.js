class SelectVariationHelper
	...
	@update_price: (element, price) ->
		if price != null
			$(element).text(price)

	@add_parameter_to_url: (param, value) ->
  ...
  @get_url_parameters: ->
  	queryString = location.search.substr(location.search.indexOf("?") + 1)
  ...
  @enable_all_option_buttons: ->
  	radio_buttons = $("input[type='radio'][data-role='variation']:not([disabled=true])")
  	radio_buttons.attr 'disabled', false
  	li = radio_buttons.parent().parent()
  	li.removeClass('unavailable').removeAttr 'title'
  
  @disable_variation_button: (variation) ->
  	options = variation.options_values
  	options_length = options.length
  	if options_length == 1
  		selector ="input[type='radio'][value='#{options[0]}']"
  		$(selector).attr('disabled', true).prop 'checked', false
  		li = $(selector).parent().parent()
  		$(li).removeClass('active').addClass('unavailable').attr('title', 'Indisponível')

  @update_image_slider: (variations) ->
